...
useEffect(() => {
  ...
  if (user) {
    ...
    // Store the FULL assignedClients array for multi-client access
    if (user.assignedClients && user.assignedClients.length > 0) {
      setUserClientName(user.assignedClients); // Store as array
      console.log(`👤 User logged in - Assigned Clients: ${user.assignedClients.join(', ')}`);
    } else if (user.clientName) {
  ...
  const fetchDemands = async () => {
    try {
      const response = await fetch("https://hrbackend-eight.vercel.app/api/demand");
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(`https://hrbackend-eight.vercel.app/api/selected-candidates/status`, requestBody);
  ...
  const handleCandidateAction = async (candidateId, newStatus, reason) => {
    try {
      ...
      const response = await axios.put(`https://hrbackend-eight.vercel.app/api/selected-candidates/status`, {
  ...
  const handleDeleteDemand = async (demandId) => {
    ...
    try {
      const response = await fetch(`https://hrbackend-eight.vercel.app/api/demand/${demandId}`, {
        method: "DELETE",
      });
...
const statusFilteredDemands = demands.filter((d) => {
  // For Client Interviewer - show Active demands for ANY assigned client
  if (userRole === "Client Interviewer") {
    // Check if demand's client is in the user's assigned clients array
    const isClientAssigned = Array.isArray(userClientName) 
      ? userClientName.includes(d.clientName)
      : d.clientName === userClientName;
    
    return d.status === "Active" && isClientAssigned;
  }
  ...
  // For Interviewer/Recruiter - show ALL statuses for assigned clients
  ...
  const handleSaveDemand = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://hrbackend-eight.vercel.app/api/demand/${editedDemand.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateCandidateStatus = async (newStatus) => {
    ...
    try {
      ...
      const statusResponse = await axios.put(`https://hrbackend-eight.vercel.app/api/selected-candidates/status`, statusRequest);
      ...
      if (statusResponse.data.success) {
        ...
        await axios.post(`https://hrbackend-eight.vercel.app/api/zone/manage`, zoneRequest);
...
<p className="text-gray-500">
  {userRole === "Client Interviewer" && userClientName && userClientName.length > 0
    ? `Showing active demands for clients: ${Array.isArray(userClientName) ? userClientName.join(', ') : userClientName}`
    : (userRole === "Interviewer" || userRole === "Recruiter") && userClientName && userClientName.length > 0
      ? `Showing demands for clients: ${Array.isArray(userClientName) ? userClientName.join(', ') : userClientName}`
                        ...
                        <div className="mb-3">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            {candidate.resumePath || candidate.googleDriveViewLink ? (
                              <a
                                href={candidate.googleDriveViewLink || `https://hrbackend-eight.vercel.app${candidate.resumePath}`}
                                target="_blank"
